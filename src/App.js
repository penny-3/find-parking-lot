...
const targetURL_parks = 'https://ga686.github.io/parking-api/parks.json'
...
class App extends Component {
  ...
  getLocations = async () => {
    const self = this
    const { data } = await axios(targetURL_parks)
    const nearest = []
    let result = data.data.park.map(park => 
      park = {
        ...park,
    ...
    for (var i = 0; i < result.length; i++) {
      ...
      if (distance( self.state.currentPos[0], self.state.currentPos[1], result[i].lat, result[i].lng, "K") <= 0.3) { 
        result[i] = {
          ...result[i],
          distance: Math.round(distance( self.state.currentPos[0], self.state.currentPos[1], result[i].lat, result[i].lng, "K")* 100)/100
        }            
        nearest.push(result[i])
      }
    }
    this.setState ({data: nearest})
  }
  ...
  componentDidUpdate(prevProps, prevState, snapshot){
    if(prevState.currentPos !== this.state.currentPos)
      this.getLocations()
  }
  ...
  render(){
    ...
    <Router>
      <Routes>
        ...
        />
        <Route path="/car" element={<Car parks={this.state.data} />} />
        <Route path="/moto" element={<Moto parks={this.state.data} />}/>
      </Routes>
    </Router>
  )
  }
